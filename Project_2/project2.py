...
class Polygons:
    ...
    def __iter__(self):
        return self.PolygonsIterator(self.circum_radius, self.num_polygons)
    
    class PolygonsIterator:
        def __init__(self, R, num_polygons):
            self.circum_radius = R
        ...
        def __next__(self):
            if self._i >= self.num_polygons:
                raise StopIteration
            else:
                self._i += 1
                return Polygon(self._i + 2, self.circum_radius)
